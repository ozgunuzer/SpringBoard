...
case when monthlymaintenance > 100 then 'expensive' else 'cheap' end as indicator
from Facilities 
...
select  distinct  concat(Facilities.name,firstname) as firstname_facname
from  Bookings
...
select concat(Facilities.name,firstname) as firstname_facname, (Bookings.slots* Facilities.guestcost) as cost
from  Bookings
join Members on Members.memid = Bookings.memid
join Facilities on Facilities.facid = Bookings.facid
...
order by (Bookings.slots* Facilities.guestcost) desc
...
(select Bookings.facid from Bookings where Bookings.starttime  >'2012-09-14'
and Bookings.starttime  <'2012-09-15')
